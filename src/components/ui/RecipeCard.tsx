...
export default function RecipeCard({ recipe, selectedIngredients }: RecipeCardProps) {
	// Calculer le pourcentage d'ingrédients que l'utilisateur a
	const totalIngredients = recipe.usedCount + recipe.missedCount;
	const matchPercentage = totalIngredients > 0 ? Math.round((recipe.usedCount / totalIngredients) * 100) : 0;
			...
			<div className="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden cursor-pointer group">
				...
				<div className="p-4">
					...
					{recipe.usedIngredients && recipe.usedIngredients.length > 0 && (
						<div className="mb-3">
							<p className="text-xs text-gray-500 mb-1">Ingrédients que vous avez :</p>
							<div className="flex flex-wrap gap-1">
								{recipe.usedIngredients.slice(0, 3).map((ingredient, index) => (
									<span key={ingredient.id} className="inline-block bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full">
										{ingredient.name}
									</span>
								))}
								{recipe.usedIngredients.length > 3 && (
									<span className="text-xs text-gray-500">+{recipe.usedIngredients.length - 3} autres</span>
								)}
							</div>
						</div>
					)}
						...
						<div className="mb-3">
							...
							<div className="flex flex-wrap gap-1">
								{recipe.missedIngredients.slice(0, 2).map((ingredient, index) => (
									<span key={ingredient.id} className="inline-block bg-orange-100 text-orange-700 text-xs px-2 py-1 rounded-full">
										{ingredient.name}
									</span>
								))}
								{recipe.missedIngredients.length > 2 && (
									<span className="text-xs text-gray-500">+{recipe.missedIngredients.length - 2} autres</span>
								)}
							</div>
						</div>
					)}
					...
					<div className="flex items-center justify-between pt-3 border-t border-gray-100">
						<div className="flex items-center space-x-2">
							<div className="flex items-center">
								{[...Array(5)].map((_, i) => (
									<svg
										key={i}
										className={`w-4 h-4 ${
											i < Math.round(matchPercentage / 20) ? 'text-yellow-400' : 'text-gray-300'
										}`}
										fill="currentColor"
										viewBox="0 0 20 20"
									>
							...
							<span className="text-sm text-gray-600">Score: {recipe.score}</span>
						</div>
